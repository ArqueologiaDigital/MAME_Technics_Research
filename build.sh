...
# SX-WSA1R control panel.  GENERATED by tools/gen_wsa1r_lay.py from Felipe's artwork;
# `python3 tools/gen_wsa1r_lay.py --check` fails if the .lay or wsa1_cpanel.cpp's
# PORT_NAMEs have drifted from the generator's matrix table.  MAME's makefile picks up
# every src/mame/layout/*.lay by wildcard, so the symlink is all the registration a new
# layout needs.
ln -sf "$HERE/src/mame/layout/wsa1r.lay"                     "$BUILD_TREE/src/mame/layout/wsa1r.lay"
