...
**Interaction survey (demo + rhythm playback) -- CLEAN.** (Corrects the prior 'blocked' note.) Panel buttons DO work; the earlier failure was a Lua-input issue -- a single-frame `set_value` tap gets cleared by the input frame-update before the 250 Hz panel scan samples it. HELD presses (set_value(1) EVERY frame) work: a held DEMO (SEG06 0x40) press changes the LCD (verified via a 0x9CE00000 screen-buffer hash 480686->538919) and enters demo mode. With held input I surveyed DEMO mode (~7.5 s, verified running) and rhythm/auto-accompaniment playback (START/STOP toggle + a held chord) -- BOTH hit **zero** unimplemented ops. So udf00 + udf07 cover boot + splash + home + idle + demo + rhythm playback: the CPU core is complete for ALL the main operational paths tested. (Method: hold inputs via manager.machine.ioport.ports[tag].fields[name]:set_value(1) each frame; run headless -video none, which is fast once ENFILE is cleared with drop-caches.)