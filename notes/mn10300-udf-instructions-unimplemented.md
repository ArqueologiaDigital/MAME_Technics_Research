...
**Attempted demo-mode survey -- BLOCKED by panel input delivery.** Pressed the DEMO button (SEG06 bit 0x40) via Lua ioport (the field set OK, ok=true) but a `-window` snapshot shows the screen **stayed on the home screen** -- demo mode did NOT start. So PANEL BUTTON presses currently do NOT reach the firmware (the driver's own note: "delivery to the firmware awaits SIO interrupts"). Therefore interaction paths (demo, rhythm/auto-accompaniment playback, menu navigation) CANNOT yet be surveyed via panel buttons -- and my earlier "demo survey = 0 ops" claim was invalid (only the home screen ran). What IS solidly surveyed and clean: boot + splash decode + home + idle. PREREQUISITE for interaction surveys: implement panel-button -> SIO-interrupt delivery in the driver (a separate task), or find a non-panel trigger (MIDI in).