...
6b52e50, `make verify` 100% byte-identical).

**READ THIS FIRST: most of 2a was already settled.** The runtime work in
`notes/rhythm-name-list-bug.md` (2026-07-07) and the shipped Phase-D
synthetic in blog Part 35 established the count (0xDD = 221), the intact
table-ROM directory, the ~4.1 MB size of the real resource, the
0x54E00000 install window, and the 52-real-names / 168-placeholders split.
This pass is a STATIC confirmation of that picture plus two small additions.
It does not change the Phase C case.

**Confirmed statically** (previously known only from runtime dumps):

- `*(0x50034B7C) = 0x483E82BF` is not a coincidence of which resource won.
  At its tail `StyleBuiltinNameTableInit` (0x484332F9) asks
  `TableRomStyleResourcePtr` (0x4843D7A8 -> table ROM directory entry
  `*(0x4800014C)` = 0x483E828C) for the in-flash copy, strncmp's the
  "Technics Rhythms" signature, and on a match **overwrites the name-table
  pointer with that copy's `u24BE(+0x1B)` regardless of which resource the
  probe won**. Count / base / offset sub-table / variation sub-table /
  fallback all still come from the winner. So the name table is structurally
  guaranteed to be the real table-ROM one — which is *why* the shipped
  synthetic's numbering lines up, and it means a future synthetic can never
  substitute its own name table even if it wanted to.
- The probe really is five signature-gated windows (0x48010000, 0x40610000,
  0x40010000, 0x54E10000, 0x40810000) plus an **unconditional** 0x54E00000
  last resort, and `StyleNameModuleBoot` (0x4843385E) falls back to the
  program-ROM stub 0x48729988 when the winner fails to open.

Where the truncation falls in `baserom/kn7000_table.rom`, for the record:
signature, header (count 0xDD) and the **full 0x800-entry name table**
(0x483E82BF, ending exactly where the sub-table begins) are intact; **177 of
221** offset-sub-table entries survive; the variation sub-table's first
record offset lands **two bytes past EOF**; **zero** of the 221 13-byte name
records are present. The surviving offsets are monotonic and reach
**+0x3E5D75 (~4.08 MB)**, consistent with the ~4.1 MB figure already on file.
...
**NEW — only ONE of the three style sources depends on the missing flash.**
The style ID's bits 0x00700000 select built-in (0), MEMORY (0x100000, via
0x484355E2 / 0x484355F8) or CUSTOM (0x200000, via 0x4848457D /
0x4843E107..0x4843E128). The MEMORY and CUSTOM paths have their own fetchers
and record layouts and never touch the undumped chip.
