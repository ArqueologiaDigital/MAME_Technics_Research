...
|---|---|---|---|
| P1 | **PCM glitch attribution.** Full plan already written: `notes/TODO-kn5000-pcm-glitch-attribution.md`. Lead hypothesis: the sustain loops are NOT in the ROM — `compute_loop()` derives them — so a loop whose length is not a whole number of periods splices at every wrap. ⚠ Re-measure the burst first: it was characterised BEFORE the INT0 fix (3fd44f3) and may have moved or gone. | yes (instrumentation) | step 3 of that plan is a static pass over all 1495 chunks needing NO emulator |
| P2 | **Per-chunk tuning trim.** The oracle showed Parts 7/11 at correct absolute pitch (z=+14.5/+9.9, peak at shift 0) and Parts 2/8/15 not. Find what 7/11 have that the others do not; if their chunks have trim 0 that identifies the undecoded per-chunk root pitch. Oracle is a ready-made pass/fail: Part 2's hit@0 should climb 0.25 -> 0.94. | no | blocked on R2 for the better per-part table |
...
6. Compute the NULL for every claim. Two hypotheses in this investigation were killed by their
   own null; one brief was factually wrong about the repo and an agent caught it.
