...
| P9 | **★★ NOTHING EVER WRITES THE PARTS WORD AFTER DEMO SETUP (2026-08-15).** Write tap on `0x10420-0x10421` (log `notes/p9-partsmask-2026-08-15.log`): **every** write happens in one burst at t=24.19–24.40, from `0xEF3FCA` / `0xEF40A9` / `0xEF4047`, ending with the pair that leaves `0x4A` at `0x10420` and `0x04` at `0x10421`. **After t=24.40 the location is never written again** — not at the stop, not in the following 145 s. So the four bits are not "stuck" in the sense of a clearing routine skipping them; **nothing touches the word at all**, and the guard keeps reading a value fixed at demo setup. <br>⚠ **My rig conflated two bytes.** The alternating `0x00XX` / `0xXX00` values are BYTE writes to `0x10420` and `0x10421` separately, appearing in the tap's 16-bit `data` with one valid lane; reading them as a u16 "mask" is wrong. `0x044A` is just `0x4A` and `0x04` side by side. Whether either byte is really a 16-part bitmap is now doubtful — the setup burst writes values like `0x64`, `0x98`, `0x50`, `0x27`, `0x1E`, which look like stream data, not flags. <br>⚠ Therefore `kn5000-docs/ssf-presentation.md`'s label — `0x10420` = "sequencer part active flags, 0xFFFF = all 16 parts active" — **does not match observed behaviour** and should be re-derived before more work leans on it. <br>**NEXT:** tap the two bytes separately, and find what `FDemo_MultiGuardCheck` actually reads — the guard may test a different location entirely, in which case this whole address is a red herring. <br>Still solid from earlier passes: the stop at t=139.47 is correct and fully traced; all voices release at the stop (`0x1CE0`→`0x0000`), so no next song is NOT caused by stuck audio. Earlier: **★★ SOFTWARE BUG, NOT A HARDWARE ONE: the voices clear and the parts do not.** Log: `notes/p9-parts-vs-voices-2026-08-15.log`. Measured in ONE run, tapping the firmware's own active-voice polls (sub-CPU `0x100000`) beside the parts mask (main-CPU `0x10420`): <br>| t | `parts[0x10420]` | active voices | <br>|---|---|---| <br>| 135–139 | `0x044A` (4 set) | `0x1CE0`, 5–7 active | <br>| **140 (stop)** | `0x044A` (4 set) | **`0x0000` — all released** | <br>| 141–199 | `0x044A` (4 set) | `0x0000` for 59 s | <br>**Every voice releases at the stop; the four sequencer parts never clear.** The firmware keeps polling the tone generator throughout (34k→36k polls), so it is alive and watching — it simply never clears those bits. <br>⛔ **This refutes two things.** (1) P9 and **P3** ("stuck EG voices … `eg_running` never clears") are **NOT the same bug** — there are no stuck voices here at all. (2) `kn5000-docs/ssf-presentation.md`'s attribution, that parts never finish *because* waveform ROMs are missing and voices stay active, is contradicted by the measurement: zero active voices, parts still set. <br>★ **Consequence: P9 is decoupled from the undumped IC304/305/306 and is fixable in software.** The defect is in the sequencer's part-completion bookkeeping — whatever should clear `DRAM[0x10420]` when a part finishes. **NEXT: a write tap on `0x10420`** — which bits get cleared, by whom, and why not bits 1, 3, 6, 10. <br>Chain for context, all verified: guard `FDemo_MultiGuardCheck` fails on the parts mask → no next song; the stop itself at t=139.47 is correct (event queue → `0xF4E720` code 1 → `0xF3CAC1` → `0xF59AB9` → `0xF5ADCA` → `0xF5AF9D` → `f5afc3`). Earlier: **★ MECHANISM FOUND: 4 STUCK SEQUENCER PARTS BLOCK THE NEXT-SONG GUARD.** Log: `notes/p9-demotimer-2026-08-15.log`. The Feature Presentation drives itself — enter state `0xE4`, `Demo_ResetCountdownTimer` sets `DRAM[0x0D2F]=15`, at 10 it calls `Demo_SelectEntry_PlaySong`, gated by `FDemo_MultiGuardCheck`, which **fails if state != `0xE4` or `DRAM[0x10420] != 0`**. Measured through t=210: <br>• `state[0x8D38]` = **`0xE4`** the whole time — the machine never leaves the presentation, so that clause passes; <br>• `parts[0x10420]` = **`0x044A`**, frozen — **exactly 4 bits set** (1, 3, 6, 10), which is precisely the "**4 stuck accompaniment parts**" `kn5000-docs/ssf-presentation.md` records, and the guard's failing clause; <br>• `countdown[0x0D2F]` = **0**, frozen, never re-armed. <br>**So the demo plays one song, stops correctly, and the next song is blocked because four sequencer parts never clear.** The stop at t=139.47 is not the bug — it is correct behaviour; the bug is that nothing follows. <br>⚠ The docs attribute the stuck parts to missing waveform ROMs. In this build IC304/305/306 are not missing but are BAD_DUMP copies of IC307, so "wrong data" rather than "no data" — whether that is sufficient to strand these four parts is the next question, and it links P9 to the undumped-waveform lane. <br>⚠ **RETRACTED from the previous entry:** "the automated demo never starts at all". That test was mis-specified — the demo timer lives INSIDE the presentation (state `0xE4`), so waiting at an idle home screen tests something that does not exist. The 300 s idle run is still valid as an observation, it just does not mean what I said. <br>Rigs: `kn5000_demotimer.lua` (guard + countdown), `kn5000_p9_stall.lua` (when), `kn5000_p9_writer.lua` (who stops it). Earlier: **★ TWO PATHS MEASURED, AND THE AUTOMATED DEMO NEVER STARTS AT ALL.** Same rig, one flag apart, so the runs are comparable: <br>• **manual** (`DEMO → LEFT 4 → LEFT 2`): one song plays t≈25→**139.47**, stops via a fully-traced correct STOP, then transport is pinned at `0x00` for the next 180 s (`notes/p9-320s-2026-08-15.log`). <br>• **automated / idle** (no input at all, 300 s, `notes/p9-idle-300s-2026-08-15.log`): **transport never leaves `0x00`** — `subtick=00`, `acc=00`, `8d38=01` against `E4` while playing. The instrument's own demo timer never fires. <br>⚠ **This contradicts `kn5000-docs/ssf-presentation.md`**, which states "the Demo Timer System (song cycling) works correctly in MAME — the timer counts down, songs cycle". Not reproduced. Bounded honestly: no automatic demo within **300 s** of idle at the home screen; a real instrument might wait longer, so this bounds rather than disproves. <br>**Consequence for P9's label:** the automated path cannot serve as the reference for "cycling works", so whether the manual path *should* cycle is still open. What is demonstrable: one song plays and stops correctly, and nothing automatic ever runs. <br>★ **The stronger lead is now the demo timer**, not the stop: a demo that never starts is a bigger defect than a demo that ends early, and it is traceable the same way (find who should post the start command to the queue at `0xF4E720`). Earlier: **★ ONE SONG PLAYS, STOPS CLEANLY, AND NOTHING FOLLOWS FOR 180 s.** A 320 s run (`notes/p9-320s-2026-08-15.log`) shows transport pinned at `0x00` from t=140 to t=320 — **the demo never advances to another song.** That refutes the "it is just a section boundary" hypothesis. <br>⚠ **CONFOUND, do not skip it:** `kn5000-docs/ssf-presentation.md` distinguishes the **automated demo path** from the **manual button-press path** (`GroupBoxProc_StartSSFPresentation`, event `0x1C00038`). My stimulus is the manual navigation `DEMO → LEFT 4 → LEFT 2`, so it may legitimately start ONE song with no cycling, and the docs' claim that "song cycling works" may apply only to the automated path. **Test both paths before calling this a defect.** <br>The chain is fully traced and every link behaves correctly: event queue → `0xF4E720` dispatcher (command code **1**) → `0xF3CAC1` teardown → `0xF59AB9` STOP thunk → `0xF5ADCA` dispatcher (table `0xF5ADF9`, index `0x1C` = `0xF5AF9D`, read from ROM) → `f5afc3 ld (0x0420),0x0c` at t=139.47. So the firmware executes a STOP it was told to perform; the open question is who posts command 1, and whether anything should post a next-song command afterwards. <br>★ Independent corroboration of a separate anomaly: the docs record the demo state machine at DRAM `0x0251D8` staying `0x0000` throughout the automated demo — the same `ssf=00` this queue's own notes flag. That belongs to the SSF **visual** path, not the audio one. Earlier: **★ THE STOP IS A QUEUED COMMAND (traced to its dispatcher).** Chain, every link verified: a command dispatcher at **`0xF4E720`** compares a code in register `L` — `1`→`call 0xf3cac1`, `2`→`f4382a`, `3`→`f437fa`, `4`→`f4384c` — and **code 1 is the end-playback teardown**. Above it is a fetch/dispatch loop (`0xf39f39`, `0xf3a342`, guarded on `0x28aa`/`0x28b4`/`0x28c5`), i.e. an event-queue processor. Below: `0xF3CAC1` teardown → `0xF59AB9` STOP thunk → `0xF5ADCA` dispatcher (table `0xF5ADF9`, index from bit 2 of `0x041e`/`0x0421`/`0x0420`; index `0x1C` = `0x00F5AF9D`, read from ROM) → `f5afc3 ld (0x0420),0x0c` at t=139.47. <br>**So nothing decides "the song ended" inline — a STOP is POSTED to the queue and the firmware executes it correctly.** The open question moves to the producer: who enqueues command 1 at t=139.47. <br>★ **Hypothesis worth testing first, because it would close P9 as not-a-defect:** the Feature Presentation is an SSF *presentation* of multiple sections, so a scripted STOP at a section boundary would be correct behaviour and "58% of the song" would be measured against the wrong total. Test by checking whether the demo restarts or advances after t=140 in a longer run, and by looking for a STOP event in the SSF/demo script data. <br>Rigs: `tools/rigs/kn5000_p9_stall.lua` (when), `kn5000_p9_writer.lua` (who writes + call stack). Earlier: **★ FULL CALL CHAIN TRACED. It is an ordinary STOP command, not a wedge.** Stack captured at the write (log `notes/p9-callstack-2026-08-15.log`; the live stack is **XSSP**, not XNSP — the CPU is in system mode, and TLCS-900 has no register called "SP"): <br>`0xF4E761` → **`0xF3CAD5`** (end-of-playback teardown) → `0xF59AB9` (public STOP thunk: `push XIZ / call / pop XIZ / ret`) → `0xF5ADCA` (dispatcher) → **`0xF5AF9D`** → `f5afc3 ld (0x0420),0x0c`. <br>The dispatcher builds an index from **bit 2 of `0x041e`/`0x0421`/`0x0420`** (`0x04`/`0x08`/`0x10`) and calls through a table at `0xF5ADF9`; with all three running the index is `0x1C`, whose entry is **`0x00F5AF9D`** — verified by reading the table out of the ROM. So this path is "stop whichever transports are running", i.e. the STOP *command*, and index `0x10` even points into the same routine's guarded tail. <br>The deciding routine `0xF3CAD5` reads `0x041e`, branches to STOP if bit 3 is set or bit 2 is clear, then runs a six-call teardown (`f3dfff`, `fe118d`, `fdf5f5`, `f3f179`, `f6e63a`, `fc8dce`) and clears `0x22FC`. <br>**NEXT: `0xF4E761`** — the level that decides the song has ended. That is where an emulation defect would live: whether the end condition is reached legitimately (the data really ends at beat 171 of 292) or spuriously. <br>⚠ A static search for callers of `0xF5AF9C` finds **zero** — the routine is entered by `calr`/table, which encodes displacements, not addresses. Static caller hunts do not work in this ROM; use the runtime stack. Earlier: **★ THE STOPPING INSTRUCTION IS `0xF5AFC3`.** A write tap on the transport word (log: `notes/p9-writer-2026-08-15.log`) finds ONE writer unique to the stop; every other writer (`0xF3ECE9`, `0xF3ECED`) echoes values all run long. Order is now unambiguous: **transport `0x0C` at t=139.47, AccPlayMode `0x00` at t=139.50** (from `0xF3CB3E`) — the stop decision comes first and the accompaniment shutdown follows it. Disassembled (`unidasm -arch tlcs900`, file offset `0x15AFC8`; program ROM maps `0xE00000-0xFFFFFF` mask `0x1FFFFF`): `f5afb2 ld (0x0421),0x0c` / `f5afb7 bit 3,(0x0420)` / `jr NZ` / `f5afbd bit 2,(0x0420)` / `jr Z` / **`f5afc3 ld (0x0420),0x0c`** / `f5afc8 ld (0x3470),0x00` / `ret`. So it is a STOP routine that sets terminal state only when the transport is running (bit 2 set) and bit 3 is clear, then clears `0x3470`. ⚠ The tap reported PC `0xF5AFC8`, the instruction AFTER the write — the tlcs900 prefetch offset the rig warns about; always disassemble before naming. **NEXT: find the CALLER.** The routine is entered with `ld A,0x86; calr 0xf5b084` just above it, and the open question is what condition at t=139.47 decides the song is over — song data, an end-of-track counter, or a spurious trigger. That is where an emulation defect would live, if there is one. | yes | instruction named; caller unknown | Exact measurement, log at `notes/p9-stall-2026-08-15.log`: transport leaves `0x04` **at t=139.48 and goes to `0x0C`** — which this project's own notes define as **terminal STOP** — then to `0x00` by t=140, with AccPlayMode `0x03`→`0x00` and the sub-tick frozen at `0x18`. Up to t=139 everything is healthy (sub-tick cycling 1C/4C/1D/4D/1E/4E). **So the transport is not freezing or hanging: the firmware reaches its own end-of-song state and stops cleanly.** That is a different bug class from "stall" — the question is not "what wedged?" but "why does the sequencer think the song ended at 58%?", which points at the song data or an end-of-track condition rather than at the interrupt/timing path. ⚠ Also: the demo now plays ~8 s LONGER than the recorded 131.5 s. ⚠ A running-maxima probe additionally reports a transport value of `0x10` somewhere in the run; the direct per-second sampling never caught it, so it is unexplained and NOT part of the pinned path. ⚠ Method: `demo_max.lua` reports MAXIMA and cannot show a transition — use `tools/rigs/kn5000_p9_stall.lua`. Earlier text: **★ THE SECOND STALL — re-measured 2026-08-15, and the recorded numbers no longer hold.** Two 150-160 s runs on the current build: the demo is **still healthy at t=139** (transport `0x04`, sub-tick cycling 18/19/1A/48/49/4A) and **stopped by t=140** (transport `0x00`, AccPlayMode `0x03`→`0x00`, sub-tick **frozen at `0x18`**). So it now plays ~8 s LONGER than the recorded 131.5 s, and a running-maxima probe shows transport passing through **`0x10`** on the way — an intermediate state the note does not mention. ⚠ Do not debug against "131.5 s" or "goes to 00": the moment has moved and the path has a step in it. Several things changed underneath since it was written (the INT0 core fix, the NVRAM correction, the detect_period bound). ⚠ Method note: `demo_max.lua` reports running MAXIMA, which is why `0x10` was invisible and the transition unobservable — use `tools/rigs/kn5000_p9_stall.lua`, which logs CURRENT values every second and announces the first departure from `0x04` with its destination. Exact transition second pending a capture in flight. Original text: **★ THE SECOND STALL.** After 3fd44f3 the demo plays 19.26 -> **131.5 s** then `transport` (0x0420) goes 04->00. That is beat 171 of 292 = **58% of the song** (was ~6%). Stuck voices precede it from t~100 s. Reproduced EXACTLY in an independent replicate, both render modes. Deterministic time + a working instrumentation path make this the best-conditioned open bug we have. | yes | the INT0 hunt's method applies directly: timestamped latch logging + LOG_HANDSHAKE |
