...
> top section, which supersedes the three hold reaffirmations below.

**Do not submit `kn5000-30`, and do not try to complete it either. Remove it.** Measured on
2026-07-22 against the February revision (`mame` @ `f8cd34a8`, i.e. months *before* the patch) and
against a build of today's tree with only this hunk removed (`v0`), on the `b3` repro at three phase
offsets, scored on pixels with the firmware's own receive ring tapped:

* **Zero benefit.** At Δ=+17 ms, the one phase of three where `kn5000-30` does not kill the link, it
  is **bit-identical to its own revert** — all six snapshot PNGs, 1106 bytes, 498 packets, 3
  malformed frames, same header histogram. Add the 12-button mapping schedule (26 snapshots,
  already byte-identical between shipped and `v0`) and the total is: **every frame in which
  `kn5000-30` does not wedge the link, it changes nothing.** No measurement anywhere in this record
  shows it mapping a press correctly where the revert maps it wrongly.
* **Session-killing cost.** Δ=0 → DEAD (`f2b583c5` ×6, 404 bytes / 147 packets). Δ=+7 ms → DEAD
  (`0e97d6c5` ×6, frozen on home, 210 bytes / 51 packets). The revert and February are LIVE at all
  three phases.
* **Root cause falsified.** February latches the identical phantom RX bytes (`if (m_rx_clock_count)`
  ungated at `tmp94c241_serial.cpp:167` in `f8cd34a8`) and maps presses correctly at Δ=0; and the
  segment-0 header count at the firmware's receive ring is **exactly 1 in all nine runs**, February,
  revert and shipped alike. The "phantom `0x00` = segment 0 = TRANSPOSE row" chain in the commit
  message is not visible at the ring in any build. Those bytes are the **orphan edges the
  architecture expects and filters at the cpanel** — as the comment at
  `tmp94c241_serial.cpp:115-122` (verbatim in February at `:110-117`, ~50 lines above the hunk)
  says it does, and warns against gating.
* **The real bug is older and untouched:** a phase-dependent packet misframe present *identically*
  in February (5–7 malformed frames per `b3`) and today (3–9). At Δ=+17 ms **February itself**
  reproduces `PIANO → RIGHT2 ORCHESTRAL PAD`, the symptom that started all of this.

Practical consequence for the series: **PR 10 has no content.** Drop `kn5000-30` from the split
rather than blocking on it, and re-open the panel-misframe question from scratch. Mainline's PR5
KN5000 panel has the same *misframe* (it predates our tree); it does **not** have our wedge, and
must not acquire it.

---

## ⛔ SUBMISSION HOLD on kn5000-30 (added 2026-07-21) — superseded by the revert recommendation above
...
actually fixed (see below). Everything else in the series is unaffected. *(2026-07-22: "correct
about phantom bytes" is now falsified — the phantom bytes were never the fault. The hold's
conclusion — do not submit — stands and hardens into a revert.)*
...
- ~~**PR 7 (NVRAM persistence)**: 18, standalone.~~ **CANCELLED** — patch 18 is the write tap that
  *caused* the "Sound Name Error" and was removed from our tree by kn5000-29 (`b1cf7db`). Submitting
  it would export the bug. There is currently **no** NVRAM-persistence patch to submit; the
  underlying power-down-transaction problem is unsolved (see patch 18's row and the
  `kn5000_power_off_nmi` bullet below).
...
- **PR 10 (control-panel serial) — ⛔⛔ EMPTY, 2026-07-22: DROP IT.** `kn5000-30` is now recommended
  for **revert**, not completion — measured bit-identical to its own revert wherever it does not
  wedge the link, and its phantom-byte root cause is falsified by the February revision, which has
  the same phantoms and maps correctly. See the revert recommendation at the top of this section.
  There is nothing left in PR 10 to submit; the panel misframe it was aimed at is a separate,
  older bug that no candidate here addressed. The paragraph below is the pre-2026-07-22 plan, kept
  for the record.

- **PR 10 (control-panel serial) — ⛔ BLOCKED** *(superseded)*: 30. Not submittable on its own; it needs a fix for
...
- `kn5000_power_off_nmi` (2 commits) adds a **core** `MACHINE_NOTIFY_POWER_OFF` machine phase.
  **★ REVISIT THIS (2026-07-21).** ~~Patch 18's write-tap achieves the same NVRAM result without
  ...
  (`b1cf7db`). With it gone there is no workaround at all, and the underlying defect is now known to
  have a second symptom: **the KN5000 warm-boot `<Db>` transpose** (a virgin NVRAM grows it on its
  own second boot with zero input — leading hypothesis, unproved, that the ROM's power-down NMI
  transaction never runs because MAME's exit path calls `eat_all_cycles()` before `nvram_save()`, so
  the saved SRAM is a mid-life snapshot). The same defect also blocks the KN5000 splash animation.
  A core power-off notification is therefore back on the table as the *only* known honest fix.
  Scheduled at `KN7000/side-quests/pending/kn5000_splash_animation.txt`; symptom write-up in
  `KN7000/side-quests/findings/kn5000_button_mapping_findings.md` (the "I am still seeing `<Db>`"
  addendum) and blog Part 72.
