...
static INPUT_PORTS_START(kn7000)
	...
	// Panel buttons organized by the sub-CPU SCAN MATRIX (kn5000 style): each CP{board}_SEG{col}
	// port is one scan segment the panel sub-CPU drives, and each bit is one SW sense line it reads.
	// This is exactly the firmware's normalized-segment (normSeg) space: the control-panel device
	// (kn7000_cpanel) forwards port n -> normSeg PORT_SEG[n] with the bit unchanged, then reverse-
	// normalizes to the wire ADDR (one ADDR per column: CPL/CPC bank11 subs 0xC0-0xCB -> segs
	...
	PORT_BIT(0x02, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("PART SELECT LEFT (SW1)")
...
void kn7000_state::kn7000(machine_config &config)
{
	...
	// Panel scan-matrix button ports (CP{board}_SEG{col}); index order matches the device's
	// PORT_SEG table (port n -> normSeg), so the device forwards each column straight to its wire.
