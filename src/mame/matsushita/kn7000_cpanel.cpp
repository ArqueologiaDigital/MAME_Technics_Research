...
// Physical (board SEG-column, SW-row) -> wire (segment, mask) translation. Built from the
// service-manual scan matrices (notes/panel-physical-scan-map.json); index [port][SW-row].
// {0,0} = no button / unbridged. Preserves each button's original wire frame after the rename.
struct phys_bind { uint8_t seg; uint8_t mask; };
static const phys_bind PHYS_MAP[22][8] = {
	{ {0x00,0x01}, {0x00,0x02}, {0x00,0x04}, {0x00,0x08}, {0x00,0x10}, {0x00,0x20}, {0x00,0x40}, {0x00,0x80} },  // CPL_SEG0
	{ {0x01,0x01}, {0x02,0x04}, {0x01,0x04}, {0x02,0x08}, {0x02,0x02}, {0x02,0x20}, {0,0}, {0x02,0x80} },  // CPL_SEG1
	{ {0x01,0x20}, {0x01,0x40}, {0x01,0x08}, {0x02,0x01}, {0x01,0x10}, {0,0}, {0x01,0x80}, {0x02,0x10} },  // CPL_SEG2
	{ {0x03,0x01}, {0x03,0x02}, {0x03,0x04}, {0x03,0x08}, {0x03,0x10}, {0x03,0x20}, {0x03,0x40}, {0x03,0x80} },  // CPL_SEG3
	{ {0x04,0x01}, {0x04,0x02}, {0x04,0x04}, {0x04,0x08}, {0x04,0x10}, {0x04,0x20}, {0x04,0x40}, {0x04,0x80} },  // CPL_SEG4
	{ {0x06,0x01}, {0x06,0x02}, {0x06,0x04}, {0x06,0x08}, {0x06,0x10}, {0x06,0x20}, {0x06,0x40}, {0,0} },  // CPL_SEG6
	{ {0x07,0x01}, {0x07,0x02}, {0x07,0x04}, {0x07,0x08}, {0x07,0x10}, {0,0}, {0,0}, {0,0} },  // CPL_SEG7
	{ {0x05,0x01}, {0x05,0x02}, {0x05,0x04}, {0x05,0x08}, {0x05,0x10}, {0x05,0x20}, {0x05,0x40}, {0x05,0x80} },  // CPC_SEG5
	{ {0x08,0x01}, {0x08,0x02}, {0x08,0x04}, {0x08,0x08}, {0x08,0x10}, {0x08,0x20}, {0x08,0x40}, {0x08,0x80} },  // CPC_SEG8
	{ {0x09,0x01}, {0x09,0x02}, {0x09,0x04}, {0x09,0x08}, {0x09,0x10}, {0x09,0x20}, {0x09,0x40}, {0x09,0x80} },  // CPC_SEG9
	{ {0x0a,0x01}, {0x0a,0x02}, {0x0a,0x04}, {0x0a,0x08}, {0x0a,0x10}, {0x0a,0x20}, {0x0a,0x40}, {0x0a,0x80} },  // CPC_SEG10
	{ {0x0b,0x01}, {0x0b,0x02}, {0x0b,0x04}, {0x0b,0x08}, {0x0b,0x10}, {0x0b,0x20}, {0x0b,0x40}, {0x0b,0x80} },  // CPC_SEG11
	{ {0x0d,0x01}, {0x0c,0x02}, {0x0c,0x04}, {0x0c,0x08}, {0x0e,0x10}, {0x15,0x04}, {0x0c,0x40}, {0,0} },  // CPR_SEG0
	{ {0x0c,0x01}, {0x0e,0x02}, {0x0d,0x04}, {0x0d,0x08}, {0x15,0x08}, {0x0c,0x20}, {0,0}, {0x0d,0x80} },  // CPR_SEG1
	{ {0x0d,0x02}, {0x0e,0x01}, {0x0e,0x04}, {0x0e,0x08}, {0x0c,0x10}, {0x0d,0x20}, {0x0e,0x40}, {0,0} },  // CPR_SEG2
	{ {0x0f,0x01}, {0x0f,0x02}, {0x10,0x04}, {0x0f,0x08}, {0x0f,0x10}, {0x0f,0x20}, {0x0f,0x40}, {0x0f,0x80} },  // CPR_SEG3
	{ {0x10,0x01}, {0x10,0x02}, {0x0f,0x04}, {0x10,0x08}, {0x10,0x10}, {0x10,0x20}, {0x10,0x40}, {0x10,0x80} },  // CPR_SEG4
	{ {0,0}, {0x11,0x02}, {0x11,0x04}, {0x11,0x08}, {0,0}, {0,0}, {0x11,0x40}, {0x11,0x80} },  // CPR_SEG5
	{ {0,0}, {0x12,0x02}, {0x12,0x04}, {0x12,0x08}, {0,0}, {0,0}, {0x12,0x40}, {0x12,0x80} },  // CPR_SEG6
	{ {0,0}, {0x13,0x02}, {0x13,0x04}, {0x14,0x08}, {0,0}, {0,0}, {0x13,0x40}, {0x13,0x80} },  // CPR_SEG7
	{ {0,0}, {0,0}, {0x0e,0x20}, {0x13,0x08}, {0,0}, {0,0}, {0,0}, {0,0} },  // CPR_SEG8
	{ {0,0}, {0,0}, {0x14,0x04}, {0x0d,0x10}, {0,0}, {0,0}, {0,0}, {0,0} },  // CPR_SEG9
};
...
TIMER_CALLBACK_MEMBER(kn7000_cpanel_device::panel_scan)
{
	...
	// Read the physical board ports and assemble each wire segment's byte via PHYS_MAP: a
	// physical (port, SW-row bit) maps to a (wire-segment, wire-mask) -- OR them in. This re-does,
	// in reverse, each sub-CPU's physical(SEG,SW) -> wire(ADDR,DATA-bit) encoding, so every button
	// still emits the identical frame it did before the CP{board}_SEG{col} port rename.
	...
	for (int p = 0; p < 22; p++)
	{
		...
		for (int b = 0; b < 8; b++)
			if (BIT(v, b))
			{
				const phys_bind &pb = PHYS_MAP[p][b];
				if (pb.mask)
					seg_state[pb.seg] |= pb.mask;
			}
	}
