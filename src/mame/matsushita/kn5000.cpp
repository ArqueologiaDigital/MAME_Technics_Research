...
static INPUT_PORTS_START(kn5000)
	...
	// discarded. 199 of the 285 words on the frame path now execute -- but only 108 of
	// them FULLY: the other 91 have their ADDRESSING executed and their arithmetic
...
ROM_START(kn5000)
	...
	// IC14 (QSIGX3C23011, 32 Mbit mask ROM).
	//
	// The read this project obtained had the device's address lines A19 and A21
	// TRANSPOSED. The service manual (page 32, "CPU SECTION (A) P.C. Diagram") shows IC14
	// wired straight: AD20<-pin44<-net A21, AD19<-pin43<-net A20, AD18<-pin2<-net A19,
	// AD17<-pin3<-net A18, AD16<-pin34<-net A17 ... i.e. AD_k <- net A(k+1) throughout,
	// exactly like IC19 beside it. So the transposition is in the DUMP, not on the board --
	// which makes the CORRECTED image the chip's actual content, and the raw read the
	// defective artifact. (AD18 and AD20 are pins 2 and 44, adjacent across the NC at pin 1:
	// the neighbourhood where a socket adapter configured for a different 44-pin part
	// mis-maps.)
	//
	// The correction is therefore applied ON DISK, once, and the file below is the chip's
	// content -- byte-identical to upstream MAME's record for this ROM. Undoing the swap
	// permutes the eight 512 KiB blocks by swapping bits 0 and 2 of the block index:
	// 0,1,2,3,4,5,6,7 -> 0,4,2,6,1,5,3,7. Verified: every style record's lane pointers must
	// land 6 bytes past a cell header "80 FF FF FF FF 87", which 3439 of 9696 do in the raw
	// read and 9696 of 9696 do in the file below. Without the correction the bad-opcode
	// watchdog trips and the accompaniment transport stops, silencing about two thirds of
	// the factory rhythms and the Feature Demo.
	//
	// The raw read is kept beside the ROM as kn5000_rhythm_data_rom.ic14.as-read-a19a21-swapped,
	// and tools/rom-record-review/ic14_descramble_check.py regenerates this file from it.
