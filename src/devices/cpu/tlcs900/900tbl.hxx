...
void tlcs900_device::op_RETI()
{
	/* INTNEST (see tlcs900.h): RETI unwinds the SR/PC frame an accepted
	   ...
	   IRQ_Epilogue writes 0 at 0xF857C3 before jumping into the scheduler
	   instead of returning, and its SWI7 dispatcher writes 0 at 0xF8E9A8 and
	   then leaves by `pop SR / ret`.  Wrapping 0 -> 0xFFFF would make the
	   next read look like a deeply nested interrupt, which is precisely the
	   answer that breaks the kernels this register exists for; clamping makes
	   the unmatched case read "not nested", which is what it is. */
	if ( m_intnest )
		m_intnest--;
...
void tlcs900_device::prepare_operands(const tlcs900inst *inst)
{
	...
	case p_CR16:
		...
		case 0x4c:  // TMP94C241
			m_p1_reg16 = &m_dmac[3].w.l;
			break;
		// INTNEST, the interrupt nesting counter (see tlcs900.h).  0x3C on the
		// TLCS-900/H, 0x7C on the /H1; the two numbers cannot collide because
		// neither family uses the other's, and both are 16-bit only -- the 8-
		// and 32-bit CR paths above and below deliberately do NOT decode it.
