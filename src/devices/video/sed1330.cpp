...
/**********************************************************************
    ...
    OVERLAY COPY -- kn7000_mame development tree.  See the note at the
    top of sed1330.h for what differs from upstream and why.
...
void sed1330_device::command_w(uint8_t data)
{
	...
	case INSTRUCTION_DISP_OFF:
		// The parameter byte is optional: a bare DISP ON / DISP OFF toggles the
		// display and leaves the FC / FP fields at whatever the last parameter
		// latched.  Upstream assigns m_d only inside data_w, so a host that
		// sends the bare form leaves the panel dark forever.  The Technics
		// SX-WSA1R's panel init ends with exactly that -- prom_a 0xF8E99A
		// ld (XIY+0x01),0x59 followed by RET.
		m_d = BIT(m_ir, 0);
	...
	case INSTRUCTION_SYSTEM_SET:
		...
		case 5:
			...
			if (clock() != 0)
			{
				...
				screen().configure(m_tcr * m_fx, m_lf, screen().visible_area(), fr);
...
void sed1330_device::draw_graphics_scanline(bitmap_ind16 &bitmap, const rectangle &cliprect, int y, uint16_t va, bool blend)
{
	// `blend` ORs into the pixel instead of replacing it, which is what layer
	// composition in all-graphics mode needs.  The width clamp is a guard
	// against a host that programs C/R wider than the screen it was configured
	// with -- upstream would write past the end of the scanline.
...
//-------------------------------------------------
//  update_graphics - all-graphics mode: every layer
//  is a 1bpp bitmap of AP bytes per line
//-------------------------------------------------

void sed1330_device::update_graphics(bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	// Reached when OVLAY selected graphics for pages 1 and 3, so pages 1, 2 and
	// 3 are all bitmaps.  Page 3 only takes part when OVLAY also asked for
	// three-layer composition (OV = 1); with OV = 0 the host does not even send
	// SAD3, although its FP field in the DISP parameter may still read "solid".
	//
	// The layers are OR-composited.  OR is the only method MX offers for three
	// layers and the only one this device implements; MX_XOR / MX_AND /
	// MX_PRIORITY_OR remain unimplemented, as they are in text mode.
	//
	// Split-screen (SL1 < L/F) and the dual-panel page 4 (WS = 1, SAD4) are not
	// modelled here: no driver in this tree exercises them in graphics mode.
	uint8_t const attr1 = m_fp & 0x03;
	uint8_t const attr2 = (m_fp >> 2) & 0x03;
	uint8_t const attr3 = (m_fp >> 4) & 0x03;

	int const lines = (m_lf < bitmap.height()) ? m_lf : bitmap.height();

	for (int y = 0; y < lines; y++)
	{
		if ((attr1 != FP_OFF) && (y < m_sl1))
			draw_graphics_scanline(bitmap, cliprect, y, m_sad1 + (y * m_ap), true);

		if (attr2 != FP_OFF)
			draw_graphics_scanline(bitmap, cliprect, y, m_sad2 + (y * m_ap), true);

		if (m_ov && (attr3 != FP_OFF))
			draw_graphics_scanline(bitmap, cliprect, y, m_sad3 + (y * m_ap), true);
	}
}
