-- floppy_load_probe.lua : boot -> DISK menu (SEG0D 0x04) -> LOAD (SEG11 0x10, R1) with a floppy
-- mounted, and log FDC accesses at 0x98010000. Run WITHOUT -log (tap prints to stdout).
...
_G._rt = sp:install_read_tap(0x98010000, 0x9801000f, "fr", function(off, d, m)
  fdc = fdc + 1
  if fdc <= 120 then emu.print_info(string.format("### FDC RD %08X pc=%08X", off, pcv())) end
end)
_G._wt = sp:install_write_tap(0x98010000, 0x9801000f, "fw", function(off, d, m)
  fdc = fdc + 1
  emu.print_info(string.format("### FDC WR %08X = %04X pc=%08X", off, d, pcv()))
end)

-- steps: {seg, mask, label, wait_frames_after}
local steps = {
  {"SEG0D", 0x04, "DISK menu",  150},
  {"SEG11", 0x10, "LOAD (R1)",  300},
}
...
_G._n = emu.register_frame_done(function()
  local ok, err = pcall(function()
    ...
    elseif phase == "hold" and fc >= 20 then
      local s = steps[si]; setbtn(s[1], s[2], 0); phase = "wait"; fc = 0
...
emu.print_info("### floppy_load_probe loaded")
