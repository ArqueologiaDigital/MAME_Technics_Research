-- SD SAVE round-trip, leg 1 (proven 2026-07-19): make the panel state distinguishable
-- (RHYTHM GROUP BALLAD), then TECHNICS FORMAT save to folder01/song01 (overwrite YES).
-- Run: DISPLAY=:0 ./run.sh -window -harddisk sdcard_work.img -autoboot_script sd_roundtrip_save.lua -seconds_to_run 55
-- (need a WRITABLE card image; the pristine dump is root-owned read-only on purpose).
-- LCD soft-key map (right column, rows 1..5):
--   LCDR1=:cpanel:CPR_SEG5 0x10, LCDR2=CPR_SEG5 0x20, LCDR3=CPR_SEG7 0x01,
--   LCDR4=CPR_SEG6 0x01, LCDR5=CPR_SEG5 0x01
-- Flow: home -> SD CARD LOAD toggle (CPR_SEG1 0x80) -> SD MENU -> LCDR2 = SD SAVE MENU
-- ("TECHNICS FORMAT" is on ROW 2 -> LCDR2 again; row 1 is BLANK, LCDR1 is a no-op there)
-- -> SD SAVE browser (SAVE=LCDR1, SONG NAMING=LCDR2, FOLDER RENAME=LCDR5) -> LCDR1 SAVE
-- -> "file already exists ... Are You Sure?" dialog (YES=LCDR3, NO=LCDR4) -> LCDR3
-- -> COMPLETED! (~15 s of card writes; FAT sectors + KN7000MN.BAK + 01001KN7.* rewritten).
