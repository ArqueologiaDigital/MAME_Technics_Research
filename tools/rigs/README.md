...
Generated by `tools/gen_rig_index.py` from each rig's own header — 128 rigs. Run any of them with `./tools/rig.sh <rig>`.
...
| `encoder_input_watch` | kn7000 *(default)* | ✔ | does keyboard/mouse input reach the ENCODER port AT ALL? |
...
| `input_seq_probe` | kn7000 *(default)* | ✔ | what is a control ACTUALLY bound to? |
...
| `kn24_barcaller` | kn2400 | ✔ | WHICH caller draws the KN2400's black text bars? |
| `kn24_bufferpoke` | kn2400 | ✔ | do the 0xFF RAM buffers actually feed the KN2400's display? |
| `kn24_dump` | kn2400 *(guessed)* |  | dump 0x9C800000..0x9C87FFFF (512KB) at t=40 to kn24_lcd.bin; also fine-scan extent. |
| `kn24_fbwrites` | kn2400 | ✔ | WHEN is the KN2400's screen actually composited? |
| `kn24_fontsrc` | kn2400 | ✔ | where does the KN2400 text drawer fetch its glyphs? |
| `kn24_glyphsrc` | kn2400 | ✔ | what memory does the KN2400's compositor actually read? |
| `kn24_planedump` | kn2400 | ✔ | dump the KN2400's UI source plane so it can be LOOKED at. |
| `kn24_planewriters` | kn2400 | ✔ | who fills the KN2400's UI source plane, and with what? |
...
| `kn5000_nmiguard` | kn5000 | ✔ | does the firmware ever ARM the power-off NMI guard by itself? |
...
| `kn5000_poweroff` | kn5000 | ✔ | does the modelled POWER switch let the firmware save its state? |
| `kn5000_saveload` | kn5000 | ✔ | does a KN5000 save-state round trip actually work? |
...
| `kn5000_warmboot` | kn5000 | ✔ | did this boot take the WARM path restored from the IC21 SRAM? |
| `kn5000_waveselect_log` | kn5000 | ✔ | which waveform chunks does the machine ACTUALLY select? |
| `kn5000_wheel_idle` | kn5000 | ✔ | what does the firmware do to the encoder SCAN LIST when NOBODY |
| `kn5000_wheel_pr_test` | kn5000 | ✔ | acceptance test for the upstream TEMPO/PROGRAM wheel PR. |
| `kn5000_wheel_rate_test` | kn5000 *(guessed)* |  | KN5000 tempo-wheel DETENT-LOSS rate test. |
| `kn5000_wheel_test` | kn5000 *(guessed)* |  | KN5000 tempo-wheel driver-path test. |
| `kn5000_wheel_test_driver_port` | kn5000 *(guessed)* |  | KN5000 tempo-wheel test, DRIVER-OWNED PORT variant. |
...
**✔** = the header documents a runnable `rig.sh` command. 56 of 128 do; the rest still need one.
