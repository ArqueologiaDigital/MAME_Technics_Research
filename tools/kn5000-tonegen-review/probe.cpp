...
int main(int argc,char**argv)
{
    ...
    for(int b=0;b<4;b++) for(int p=0;p<4;p++){
        ...
        // period stats on the big ones
        uint32_t bad=0;
        for(uint32_t i=0;i<n;i++){
            /*all*/;
            const char*why; uint32_t per=detect_period(d.pcm_start[i],d.pcm_samples[i],&why);
            if(strcmp(why,"ac")!=0){ if(0)printf("   chunk %u samples=%u -> fallback '%s' per=%u (true samples<<16 would be %llu)\n",
                i,d.pcm_samples[i],why,per,(unsigned long long)(uint64_t(d.pcm_samples[i])<<16)); bad++; }
            ;
        }
    }
    return 0;
